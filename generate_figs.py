...
def compare(ds1, ds2, ds3, ranges, bins=50, title='', name='hist'):
	...
	for i in range(4):
		...
		plt.hist(ds1[i], bins=bins, range=[ranges[i,0], ranges[i,1]], histtype='step', normed=True, color='red', linewidth=1.5, label=r'$R_{2\rightarrow 2}$' if i==0 else ' ')
		plt.hist(ds2[i], bins=bins, range=[ranges[i,0], ranges[i,1]], histtype='step', normed=True, color='green', linewidth=1.5, label=r'$R_{2\rightarrow 2} + R_{2\rightarrow 3}$' if i==0 else ' ')
		plt.hist(ds3[i], bins=bins, range=[ranges[i,0], ranges[i,1]], histtype='step', normed=True, color='blue', linewidth=1.5, label=r'$R_{2\rightarrow 2} + R_{2\rightarrow 3} + R_{3\rightarrow 2}$' if i==0 else ' ')
		if i==0:
