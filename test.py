...
#e1 = event.event(mode='static', transport='LGV', static_dt=1., elastic=True, inelastic=False, detailed_balance=False, EinR=True, mass=1.3)

# Dynamic Meidum
#e1 = event.event(mode='dynamic', hydrofile=sys.argv[2], inelastic=True, detailed_balance=True, mass=1.3)
e1 = event.event(mode='dynamic', transport='LGV', hydrofile=sys.argv[2], elastic=True, inelastic=False, detailed_balance=False, EinR=False, mass=1.3)
f = h5py.File("particledata_test_LGV.hdf5", 'w')

# Initialize HQ pT weight and xy-distribution

TR = np.loadtxt(sys.argv[1]).T ## p=0.   ##? why you need to rotate TR?
Taa = TR**2

pt, dsigma = np.loadtxt(sys.argv[3], unpack=True)
dfdpt2 = interp1d(pt, dsigma)
def pT_weight(pt, y):
	return dfdpt2(pt)

e1.initialize_HQ(NQ=1000000, Taa=Taa, dxy=0.1, df_dpt2dy=pT_weight, oversample_power=-1.)
