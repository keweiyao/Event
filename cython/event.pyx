...
cdef class event:
        ...
        cpdef initialize_HQ(self, NQ, init_flags):
                ...
                if init_flags['type'] == 'A+B':   
                        ...
                        df_dpt2dy = init_flags['df_dpt2dy']
                        if df_dpt2dy == None:
                                print "use the default weight"
                        ...
                        while it != self.active_HQ.end():
                                ...
                                deref(it).weight = pT**(2. - 1./alpha)*df_dpt2dy(pT, y)
                ...
                if init_flags['type'] == 'test':
                        print "Initialize for test simulation"
                        pmax = init_flags['pmax']
                        L = init_flags['L']
                        it = self.active_HQ.begin()
                        while it != self.active_HQ.end():                                          
                                ...
                                deref(it).p.resize(4)
                                deref(it).x.resize(4)
                                deref(it).p = [sqrt(p**2+self.M**2), 0.0, 0.0, p*cospz]                   
                                deref(it).t_last = 0.0; deref(it).t_last2 = 0.0
                                deref(it).Nc = 0; deref(it).Nc2 = 0
                                deref(it).count22 = 0; deref(it).count23 = 0; deref(it).count32 = 0
                                deref(it).weight = 1.0
        ...
        cdef perform_HQ_step(self, vector[particle].iterator it): 
                ...
                if T < 0.154:  # already in the hadronization surface
                        channel = 20
                        dtHQ = 0.02
                        pnew = deref(it).p
                        freestream_stay(it, dtHQ)
                        deref(it).p = pnew
                else:
        ...
        cdef update_HQ_LGV(self, vector[double] p1_lab, vector[double] v3cell, double Temp) :
                ...
                dtHQ = p1_lab[0]/p1_cell[0] * dt_cell  #? what is this for??
              
                channel = 10  #? need to change this, reserve a spectial number for Langevin transport
                if p1_cell[0] > 1e6:
                        print 'event error?: ', p1_cell, p1_cell_Z_new, pnew
